...
__global__ void gpu_compute_forces(uint points, scalar_type* force_factors, vec_type<scalar_type,WIDTH>* density_deriv, vec_type<scalar_type,WIDTH>* forces, uint nucleii_count)
{
	uint atom = index_x(blockDim, blockIdx, threadIdx);
	bool valid_thread = (atom < nucleii_count);

	vec_type<scalar_type,WIDTH > atom_force(0.0f,0.0f,0.0f);
  ...
  for (uint point_base = 0; point_base < points; point_base += FORCE_BLOCK_SIZE) {
      ...
      for (uint point_sub = 0; point_sub < FORCE_BLOCK_SIZE && (point_base + point_sub < points); point_sub++) {
        vec_type<scalar_type,WIDTH> density_deriv_local(density_deriv[COALESCED_DIMENSION(points) * atom + (point_base + point_sub)]); // uncoalesced
